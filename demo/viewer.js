...
function show3DContextMenu(compositeKey, x, y) {
  ...
  // Check if any selected elements are installed
  ...
  // Wire production tracking items
  for (let pi = 0; pi < prodItems.length; pi++) {
    const prodEl = menu.querySelector(`[data-action="prod-${pi}"]`);
    if (prodEl) {
      const action = prodItems[pi].action;
...
// ======================== AUTO-LOAD IFC FILES FROM DIRECTORY ========================

/**
 * Auto-discover and load .ifc files co-located with index.html.
 * Strategy:
 *   1. Try fetching a directory listing (works with python -m http.server, etc.)
 *      and parse <a> tags pointing to .ifc files.
 *   2. Fall back to a models.json manifest: [{ "file": "Model.ifc" }, ...]
 *   3. If neither works, do nothing — the user can still drag-and-drop.
 */
async function autoLoadModels() {
  let ifcUrls = [];

  // --- Strategy 1: Parse HTTP directory listing ---
  try {
    const resp = await fetch('./', { method: 'GET' });
    if (resp.ok) {
      const html = await resp.text();
      // Look for links ending in .ifc (case-insensitive)
      const linkPattern = /href="([^"]+\.ifc)"/gi;
      let match;
      while ((match = linkPattern.exec(html)) !== null) {
        const href = match[1];
        // Skip absolute URLs to other domains
        if (!href.startsWith('http') || href.startsWith(window.location.origin)) {
          ifcUrls.push(decodeURIComponent(href));
        }
      }
    }
  } catch (e) {
    console.log('[viewer] Directory listing not available:', e.message);
  }

  // --- Strategy 2: Fall back to models.json manifest ---
  if (ifcUrls.length === 0) {
    try {
      const resp = await fetch('./models.json');
      if (resp.ok) {
        const manifest = await resp.json();
        // Support both ["file.ifc", ...] and [{ "file": "file.ifc" }, ...]
        for (const entry of manifest) {
          if (typeof entry === 'string') {
            ifcUrls.push(entry);
          } else if (entry.file) {
            ifcUrls.push(entry.file);
          }
        }
      }
    } catch (e) {
      console.log('[viewer] No models.json found:', e.message);
    }
  }

  if (ifcUrls.length === 0) {
    console.log('[viewer] No IFC files found to auto-load.');
    return;
  }

  console.log(`[viewer] Auto-loading ${ifcUrls.length} IFC file(s):`, ifcUrls);

  // Fetch each file and build the files array for loadIFCFiles
  const files = [];
  for (const url of ifcUrls) {
    try {
      const resp = await fetch(url);
      if (!resp.ok) {
        console.warn(`[viewer] Failed to fetch ${url}: ${resp.status}`);
        continue;
      }
      const buffer = await resp.arrayBuffer();
      // Extract filename from URL
      const name = decodeURIComponent(url.split('/').pop());
      files.push({ name, buffer, size: buffer.byteLength });
    } catch (e) {
      console.warn(`[viewer] Error fetching ${url}:`, e.message);
    }
  }

  if (files.length > 0) {
    await loadIFCFiles(files, false);
  }
}
...
initThreeJS();
wireEvents();
initIfcAPI().then(() => autoLoadModels());
